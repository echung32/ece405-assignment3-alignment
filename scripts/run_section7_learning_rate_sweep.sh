...
CAMPAIGN_NAME="${1:-section7_grpo_learning_rate_$(date +%Y%m%d_%H%M%S)}"
shift $(( $# > 0 ? 1 : 0 )) || true
...
RUN_CONFIGS=(
    "label=reinforce_lr5e-6_std lr=5e-6 loss=reinforce_with_baseline std=1 epochs=1 train_batch=256 grad_accum=128 steps=200"
    "label=reinforce_lr3e-5_std lr=3e-5 loss=reinforce_with_baseline std=1 epochs=1 train_batch=256 grad_accum=128 steps=200"
    "label=reinforce_lr1e-5_std lr=1e-5 loss=reinforce_with_baseline std=1 epochs=1 train_batch=256 grad_accum=128 steps=200"
)
...
for config in "${RUN_CONFIGS[@]}"; do
    unset run_label learning_rate loss_type use_std epochs_per_rollout_batch train_batch_size gradient_accumulation_steps n_grpo_steps
    for kv in $config; do
        key="${kv%%=*}"
        value="${kv#*=}"
        case "$key" in
    ...
    echo "[start] run=${run_label} lr=${learning_rate} loss=${loss_type} std=${use_std} steps=${n_grpo_steps} $(date -Is)" | tee -a "$ORCHESTRATOR_LOG" | tee -a "$run_log"
