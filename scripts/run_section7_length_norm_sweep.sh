...
CAMPAIGN_NAME="${1:-section7_grpo_lengthnorm_$(date +%Y%m%d_%H%M%S)}"
shift $(( $# > 0 ? 1 : 0 )) || true
...
for config in "${RUN_CONFIGS[@]}"; do
    unset run_label learning_rate loss_type reward_function prompt_path length_normalization length_normalize_constant use_std epochs_per_rollout_batch train_batch_size gradient_accumulation_steps n_grpo_steps
    for kv in $config; do
        key="${kv%%=*}"
        value="${kv#*=}"
        case "$key" in
    ...
    echo "[start] run=${run_label} loss=${loss_type} reward=${reward_function} length_norm=${length_normalization} steps=${n_grpo_steps} $(date -Is)" | tee -a "$ORCHESTRATOR_LOG" | tee -a "$run_log"
