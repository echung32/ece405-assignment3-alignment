...
CAMPAIGN_NAME="${1:-section7_grpo_no_clip_ablation_$(date +%Y%m%d_%H%M%S)}"
shift $(( $# > 0 ? 1 : 0 )) || true

BEST_EPOCHS="${BEST_EPOCHS:-4}"
BEST_TRAIN_BATCH="${BEST_TRAIN_BATCH:-128}"
BEST_GRAD_ACCUM="${BEST_GRAD_ACCUM:-64}"
...
echo "[launch] campaign=${CAMPAIGN_NAME} log_dir=${LOG_DIR} output_root=${OUTPUT_ROOT} best_epochs=${BEST_EPOCHS} best_train_batch=${BEST_TRAIN_BATCH} best_grad_accum=${BEST_GRAD_ACCUM}" | tee -a "$ORCHESTRATOR_LOG"
